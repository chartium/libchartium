...
<script lang="ts">
  ...
  const dragCallbacks: MouseDragCallbacks = {
    ...
    move: (e) => {
      zoomOrMove = "move";
      transformPosition!.to = axis === "x" ? e.offsetX + axisOffset : e.offsetY;
    },
    end: (e) => {
      updateRange();
      zoomOrMove = "neither";
  ...
  >
    ...
    {#if transformPosition !== undefined && transformPosition.from !== transformPosition.to}
      {#if axis === "x"}
        ...
        <div class="tooltip" style:left="{transformPosition.to - axisOffset}px">
      ...
      {:else}
        ...
        <div class="tooltip" style:top="{transformPosition.to - axisOffset}px">
          {transformValue?.to.toFixed(3)}
        </div>
      {/if}
    {/if}
